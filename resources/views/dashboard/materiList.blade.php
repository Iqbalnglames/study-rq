...
@section('content')
    <div class="">
        @if (session()->has('success'))
            <div class="font-regular fixed block w-72 max-w-screen-md rounded-lg right-5 top-5 bg-green-500 px-4 py-4 text-base text-white"
                data-dismissible="alert" id="alert">
        ...
        <a href="/dashboard/buat-materi" class="p-2 bg-green-600 rounded text-white border border-gray-300">+ Buat
            Materi</a>
        ...
        <table class=" border border-gray-300 rounded-md mt-4 w-full overflow-y-scroll">
            ...
            @foreach ($materials as $material)
                <tr class="">
                    <td class="border p-3 border-gray-300">
                        <h1>{{ $material->title }}</h1>
                    </td>
                    <td class="border p-3 border-gray-300">
                        <h1>{{ $material->classLevel->name }}</h1>
                    </td>
                    <td class="border p-3 uppercase text-center border-gray-300">
                        <a href="edit-materi/{{ $material->slug }}" class="bg-blue-500 p-2 rounded text-white">edit</a>
                        <a href="#" onclick="openModal('{{ $material->title }}', {{ $material->id }} )"
                            class="bg-red-500 p-2 rounded text-white">delete</a>
                    </td>
                </tr>
