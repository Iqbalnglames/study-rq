...
@section('content')
    <form action="{{ route('update-materi', $editMateri->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="space-y-3">
            <div class="flex flex-col">
                <label for="title">Judul Materi</label>
                <input type="text" class="p-2 rounded-sm w-96 border border-slate-300 focus:ring-blue-500" name="title"
                    placeholder="Tulis judul materi di sini" value="{{ $editMateri->title }}">
            </div>
            ...
            <button type="submit" class="p-2 rounded-md bg-green-500 border border-slate-300 text-white">Update
                Materi</button>
