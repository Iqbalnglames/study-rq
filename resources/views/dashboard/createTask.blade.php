    ...
    <div class="px-10 pt-2">
        ...
        <form class="space-y-2" method="POST" action="{{ route('/buat-tugas') }}" enctype="multipart/form-data">
            ...
            <div class="flex flex-col">
                ...
                <input name="question_title" type="text" class="border-b p-2 bg-white border rounded">
            </div>
            <div class="flex flex-col">
            ...
            <div class="flex flex-col">
                <select name="answer_type" id="answer_type" class="border-b p-2 bg-white border rounded">
                    <option>-- Pilih Tipe Jawaban --</option>
                    @foreach ($answerType as $answerType)
                        <option value="{{ $answerType->value }}">
                            {{ $answerType->value == 'image' ? 'upload gambar' : 'pilihan ganda' }} </option>
                    @endforeach
                </select>
            </div>
            <div id="answers" class="mt-4">
                {{-- tempat untuk menaruh element secara dinamis --}}
            </div>
...
<input class="border-b w-96 outline-none focus:border-blue-500 focus:border-b-2" type="text" name="answers[]" value="" class="form-control">
        ...
        multiple_choices.addEventListener('change', function() {
            if (multiple_choices.value === 'multiple_choices') {
