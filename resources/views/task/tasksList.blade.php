...
@section('content')
    <div style="background-image: url('{{ asset('images/class-bg.jpg') }}'); " class="h-screen bg-cover z-0 relative">
        <div class="bg-black h-screen bg-opacity-60 overflow-scroll">
            <h1 class="text-center text-2xl font-bold py-4 text-white">Pilih paket tugas</h1>
            <div class="grid grid-cols-1 lg:px-0 lg:grid-cols-3 gap-y-10 mb-10 place-items-center">
                @foreach ($tasks as $task)
                    <a href="/penugasan/{{$materialTarget->slug}}/{{ $task->slug }}">
                        <div
                            class="flex hover:bg-gray-100 bg-white flex-col w-52 h-56 shadow-xl items-center rounded-md border border-gray-200">
                            <img class="rounded-t-md" src="{{ asset('images/book-bg.jpg') }}" alt="gambar-buku">
                            <h1 class="text-center">{{ $task->question_title }}</h1>
                        </div>
                    </a>
                @endforeach
            </div>
            <div class="grid grid-rows-1 place-items-center">
                <a href="/penugasan" type="button"
                    ...
                    Kembali ke menu Materi Penugasan
                </a>
            </div>
        </div>
    </div>
